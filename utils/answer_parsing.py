import struct
from new_version.devices.arduino_board import ArduinoBoard
    ...
    if message.startswith(RECIEVE_HEADER):
        ...
        # Читаем код команды
            if offset + 2 > len(message):
                ...
                break
            cmd = struct.unpack_from("<H", message, offset)[0]  # <h = little-endian signed short
            offset += 2
            ...
            if cmd == arduino.CMD_READ_FREQUENCES:
                if offset + 2 > len(message):
                    ...
                    break
                count = struct.unpack_from("<H", message, offset)[0]  # <H = little-endian unsigned short
                offset += 2
                ...
                error_code = struct.unpack_from("<H", message, offset)[0]  # <H = little-endian unsigned short
                offset += 2
